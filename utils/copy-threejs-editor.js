...
const studioDir = "./bla15e-studio"


async function build() {
    console.log(path.relative(editorDir, studioDir))
    console.log("Cloning editor directory")

    process.chdir(editorDir)
    for(var i in assets) {
        let assetPath = assets[i]
        let {base, dir} = path.parse(assetPath)

        // Copy Editor Assets to bla15e-studio
        if(dir.startsWith("./")) {
            // Handle base files
            console.log(`Inside dir ${assetPath}`)
            await fs.copy(assetPath, path.join('../bla15e-studio', assetPath))

        } else if (dir.startsWith('../')) {
            // Handle files not inside the editor directory
            console.log(`Outside dir ${assetPath}`)
            await fs.copy(assetPath, path.join('../bla15e-studio/lib', base))
        }
    }
}
